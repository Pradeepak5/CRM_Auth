...
export default function Admin_AddEmployee() {

  const navigate = useNavigate();

  const EmployeeFormValidateSchema = object({
    name: string().required("Please enter Employee name"),
    email: yup.string().email().required("Please enter email"),
    mobile: string().required("enter Employee mobile number"),
  ...
  const { values, handleChange, handleBlur, handleSubmit, touched, errors } = useFormik({
    ...
    onSubmit: (newEmployee) => {
      AddNewManager(newEmployee);
      navigate('/adminemployee');
    }
  });

  const AddNewManager = async (newEmployee) => {
    console.log(newEmployee)
    try {
        const res = await axios.post(`${url}/admin/addemployee`, newEmployee);
  ...
  return (
    <div>
      ...
      >
        ...
        <button type="button" className='btn btn-danger' onClick={()=>navigate('/adminemployee')}>Cancel</button>
